...
.equ b1, 0b1                      @ Botao 0
.equ b2, 0b10                     @ Botao 1
.equ b3, 0b100                    @ Botao 2
.equ b4, 0b1000                   @ Botao 3
.equ b5, 0b10000                  @ Botao 4
.equ b6, 0b100000                 @ Botao 5
.equ b7, 0b1000000                @ Botao 6
.equ b8, 0b10000000               @ Botao 7
.equ b9, 0b100000000              @ Botao 8
.equ b10, 0b100000000             @ Botao 9
.equ b11, 0b1000000000            @ Botao 10
.equ b12, 0b10000000000           @ Botao 11
.equ b13, 0b100000000000          @ Botao 12
.equ b14, 0b1000000000000         @ Botao 13
.equ b15, 0b10000000000000        @ Botao 14
.equ b16, 0b100000000000000       @ Botao 15
...
mov r0,#4       @ Seta a posicao do display onde a mensagem vai ser lida
mov r1,#9
ldr r2,=mensagem_inicial_9
swi Print_a_string_in_Display
mov r0,#4       @ Seta a posicao do display onde a mensagem vai ser lida
mov r1,#10
ldr r2,=mensagem_inicial_10
swi Print_a_string_in_Display
mov r0,#4       @ Seta a posicao do display onde a mensagem vai ser lida
mov r1,#11
...
cmp r9,#8
beq operacao_arranjo
cmp r9,#9
beq operacao_combinacao
...
mov r1,#5
ldr r2,=simbolo_combinacao
swi Print_a_string_in_Display
mov r0,#17                        @ Seta a posicao do display onde a mensagem vai ser lida
mov r1,#5
mov r2,r8      
swi Print_value_in_Display       @ Imprime o valor 1 no dispaly
mov r0,#18                        @ Seta a posicao do display onde a mensagem vai ser lida
mov r1,#5
ldr r2,=simbolo_igual
swi Print_a_string_in_Display
mov r3,r4                       @ O r3 vai ser usado como auxiliar no fatorial
mov r7,r8                       @ Salva o segundo valor digitado para calcular o fatorial do mesmo
sub r8,r4,r8                    
fatorial_1_comb:
    sub r3,r3,#1                
    mul r4,r3,r4
    cmp r3,#1
bgt fatorial_1_comb
mov r6,r4                       @ Salva o valor do fatorial do primeiro valor digitado
mov r3,r8                       @ O r3 vai ser usado como auxiliar no fatorial
mov r4,r8                       
fatorial_2_comb:                
    sub r3,r3,#1                
    mul r4,r3,r4                @ Salva o valor do fatorial da diferença
    cmp r3,#1
bgt fatorial_2_comb     
mov r8,r7                       @ Recebe o segundo valor digitado (que foi salvo antes em r7) para calcular o fatorial do mesmo    
mov r3,r7
fatorial_3_comb:                @ Fatorial do segundo termo
    sub r3,r3,#1                
    mul r8,r3,r8
    cmp r3,#1
bgt fatorial_3_comb
mul r7,r8,r4                    @Produto dos fatoriais do denominador
mov r3,#0                       @ O r3 serve como o quociente da divisão
cmp r6,r7 	
blt desviodivisao 
divisao_1_comb:
    ...
    cmp r6,r7
    bge divisao_1_comb
mov r2,r3                        @ Move o resultado do quociente para ser printado
mov r0,#5                        @ Seta a posicao do display onde a mensagem vai ser lida
mov r1,#6
...
mov r0,#5                        @ Seta a posicao do display onde a mensagem vai ser lida
mov r1,#5
ldr r2,=simbolo_arranjo
swi Print_a_string_in_Display
mov r0,#14                        @ Seta a posicao do display onde a mensagem vai ser lida
mov r1,#5
mov r2,r8      
swi Print_value_in_Display       @ Imprime o valor 1 no dispaly
mov r0,#4                        @ Seta a posicao do display onde a mensagem vai ser lida
...
swi Print_a_string_in_Display
mov r3,r4                       @ O r3 vai ser usado como auxiliar no fatorial
sub r8,r4,r8                    @ Salva o valor para se calcular o segundo fatorial
fatorial2:
    sub r3,r3,#1                
    mul r4,r3,r4
    cmp r3,#1
bgt fatorial2
mov r6,r4                       @ Salva o valor do primeiro fatorial
mov r3,r8                       @ O r3 vai ser usado como auxiliar no fatorial
mov r4,r8                       @ Fatorial da diferença
fatorial3:
    sub r3,r3,#1                
    mul r4,r3,r4
    cmp r3,#1
bgt fatorial3
mov r8,r4                       @ Salva o valor do segundo fatorial
mov r3,#0                       @ O r3 serve como o quociente da divisão
cmp r6,r8 	
blt desviodivisao 
divisao_1_arranjo:
...
.data
mensagem_inicial_1: .asciz "Calculadora de 8 operacoes - IFES"
...
mensagem_inicial_9: .asciz "7 - Arranjo simples"
mensagem_inicial_10: .asciz "8 - Combinacao "
...
simbolo_igual: .asciz "="
simbolo_arranjo: .asciz " ARRANJO "
simbolo_combinacao: .asciz " COMBINACAO "
